...
describe('TimerDigits component', () => {
  describe('more than one minute remaining', () => {
    const component = <TimerDigits secondsRemaining={1499} sessionLengthMins={25} />

    test('renders the minutes', () => {
      const { getByLabelText } = render(component)
      const minsRemainingElement = getByLabelText(/Minutes remaining/i)
      expect(minsRemainingElement).toHaveTextContent('24')
    })
    
    test('renders the seconds', () => {
      const { getByLabelText } = render(component)
  ...
  describe('less than one minute remaining', () => {
    const component = <TimerDigits secondsRemaining={58} sessionLengthMins={25} />
  ...
  describe('zero seconds remaining', () => {
    const component = <TimerDigits secondsRemaining={0} sessionLengthMins={5} />
