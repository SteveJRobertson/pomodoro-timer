...
const TimerDigits: React.FC<TimerDigitProps> = ({secondsRemaining, sessionLengthMins}) => {
  ...
  const getDisplayValue = (num: number) => (`${num}`.length === 1 ? `0${num}` : `${num}`);

  return (
    <div css={timerStyles}>
      <div
        aria-label="Minutes remaining"
        css={timerDigitStyles}
      >
